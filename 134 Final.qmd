...
For this part, we will work with some essay data from our dataset review column. 

To achieve this process with nlp process, first step is to do some transformations and cleaning. 
...
# clean  remove non-digit characters 
amazon$discount_percentage <- as.numeric(gsub("[^0-9]", "", amazon$discount_percentage))

amazon$rating_count <- as.numeric(gsub("[^0-9.]", "", amazon$rating_count))
options(scipen = 999) # taking of scientific cnotation
...
ggplot(amazon, aes(x = actual_price, y = discounted_price)) +
  geom_point(alpha = 0.5, color = "red") +
  geom_smooth(method = "lm", color = "darkred", se = FALSE) +
  labs(title = "Discounted Price vs Actual Price",
       x = "Actual Price",
       y = "Discounted Price") +
  theme_minimal()
...
The second graph illustrates the relationship between discounted price and actual price. This graph reveals a much stronger positive correlation as the actual price increases the the discounted price increases as well. Finally, when comparing the discounted percentage with ratings, the trend line exhibits only a very slight negative slope, suggesting that lower ratings are modestly associated with higher discount percentages.

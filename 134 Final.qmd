...
```{r}
ggplot(amazon, aes(x = actual_price, y = discount_percentage)) +
...
The warning message indicates that 934 rows were removed due to missing values or values outside the expected range.
This likely means that some values in actual_price or discount_percentage are NA or improperly formatted.
...
-   Key Observations:
    ...
    -   Many discounts are grouped around ₹500, and ₹1000, suggesting that retailers tend to apply similar discount strategies for certain price ranges.

    -   Higher Prices Have Higher Discount

        -   This suggests that expensive products often receive bigger percentage discounts, likely to make them more attractive

    -   Low Prices Have Lower Discounts

        -   Products under ₹250 mostly have discounts below 25%, indicating that cheaper items may already be priced competitively.
